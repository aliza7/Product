...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['manage_comment'])) {
    if (isset($_POST['post_id'])) {
        $postId = $_POST['post_id'];

        // Fetch post details
        $sql = $conn->prepare("SELECT * FROM post_details WHERE id = ?");
        ...
        if ($result->num_rows > 0) {
            ...
            if ($_SESSION['user_role'] == 1) {
                $commentsSql = $conn->prepare("SELECT post_comments.*, user_details.name AS username FROM post_comments INNER JOIN user_details ON post_comments.user_id = user_details.id WHERE post_id = ?");
                $commentsSql->bind_param("i", $postId);
            } else {
                $commentsSql = $conn->prepare("SELECT post_comments.*, user_details.name AS username FROM post_comments INNER JOIN user_details ON post_comments.user_id = user_details.id WHERE post_id = ? AND post_comments.user_id = ?");
                $commentsSql->bind_param("ii", $postId, $user_id);
            }
            $commentsSql->execute();
...
elseif ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_comment'])) {
    ...
    $deleteSql = $conn->prepare("DELETE FROM post_comments WHERE id = ?");
...
<body>
    ...
    <div class="container">
        <h2 class="mt-5 mb-4" style="padding-top: 50px;">Manage Comments for <?php echo $postTitle; ?></h2>
        <div class="row ">
            <?php
